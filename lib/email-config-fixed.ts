// Direct Nodemailer implementation to bypass import issues
...
export function getEmailTransporter() {
  try {
    ...
    const config = emailConfig[provider as keyof typeof emailConfig] || emailConfig.mailgun
    
    // Use the credentials from .env.local
    const user = process.env.SMTP_USER || process.env.MAILGUN_USER
    const pass = process.env.SMTP_PASS || process.env.MAILGUN_PASS
    
    if (!user || !pass) {
      console.warn('⚠️ Email credentials not configured. Using mock transporter.')
      console.log('🔧 To enable real emails, set SMTP_USER and SMTP_PASS in .env.local')
      return getMockTransporter()
    }
    
    // Override config with actual credentials
    const finalConfig = {
      ...config,
      auth: {
        user: user,
    ...
    return nodemailer.createTransporter(finalConfig)
  } catch (error) {
